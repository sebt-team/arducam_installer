...
configCheck() {
    ...
    if [ $? -ne 0 ]; then
        ...
        sudo apt install -y --allow-downgrades i2c-tools
    fi
...
if [[ $VERSION_CODENAME == "bookworm" ]]; then
    ...
    if [[ $package == *"kernel_driver"* ]]; then
        ...
        sudo apt --reinstall install -y --allow-downgrades ./$pkg_name
        ...
        if [ $package == 'libcamera_bookworm' ]; then
            ...
            sudo apt --reinstall install -y --allow-downgrades ./$pkg_name_dev
            sudo apt --reinstall install -y --allow-downgrades ./$pkg_name_ipa
            sudo apt install -y --allow-downgrades python3-libcamera
            sudo apt install -y --allow-downgrades python3-picamera2
        fi
    fi
